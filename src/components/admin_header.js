...
import { Col, Row, RangeSlider } from '@nio/ui-kit';
...
class AdminHeader extends React.Component {
  ...
  handleSystemStatsData = (data) => {
    const { cpu_changing, up_changing, down_changing, ram_changing } = this.state;
    ...
    if (cpu_limit !== undefined && !cpu_changing) {
      this.setState({ cpu_limit });
    }
    if (up_limit !== undefined && !up_changing) {
      this.setState({ up_limit });
    }
    if (down_limit !== undefined && !down_changing) {
      this.setState({ down_limit });
    }
    if (ram_limit !== undefined && !ram_changing) {
  ...
  changeLimit = (newLimit) => {
    if (this.brewer) {
      this.setState(newLimit);
    }
  };

  sendBrew = (data) => {
    const { forceClientUpdate } = this.props;
...
stressed devices
            ...
            <Col xs="6" sm="3" className="summary-label border-right">
              <h4 className="m-0">
                {Math.round(available_cpu)}
/
                {Math.round(total_cpu)}
              </h4>
available vCPU
            </Col>
            <Col xs="6" sm="3" className="summary-label border-right">
              <h4 className="m-0">
                {Math.round(available_ram)}
/
                {Math.round(total_ram)}
              </h4>
GB available RAM
            </Col>
            <Col xs="6" sm="3" className="summary-label">
              <h4 className="m-0">
                {Math.round(available_disk)}
/
                {Math.round(total_disk)}
              </h4>
GB available DISK
            </Col>
          </Row>
          ...
          <Row className="mb-2">
            ...
            <Col xs="6">
              <RangeSlider
                ...
                onChange={v => this.changeLimit({ cpu_limit: v })}
              />
            </Col>
          ...
          <Row className="mb-2">
            ...
            <Col xs="6">
              <RangeSlider
                ...
                onChange={v => this.changeLimit({ ram_limit: v })}
              />
            </Col>
            <Col xs="3" className="text-right text-nowrap">
              {ram_limit}
          ...
          <Row className="mb-2">
            ...
            <Col xs="6">
              <RangeSlider
                ...
                onChange={v => this.changeLimit({ down_limit: v })}
              />
            </Col>
            <Col xs="3" className="text-right text-nowrap">
              {down_limit}
          ...
          <Row className="mb-1">
            ...
            <Col xs="6">
              <RangeSlider
                ...
                onChange={v => this.changeLimit({ up_limit: v })}
              />
            </Col>
            <Col xs="3" className="text-right text-nowrap">
              {up_limit}
...
export default withPubkeeper(AdminHeader);
