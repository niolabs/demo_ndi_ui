...
# sudo curl http://dni.demo.niolabs.com/install/install_mac.sh | bash
##########################################################################
...
sudo pip3 install -qqq --upgrade pip
sudo pip3 install -qqq psutil
sudo pip3 install -qqq pubkeeper.server.core-0.2.5-py3-none-any.whl
sudo pip3 install -U -qqq safepickle
...
if [ $nio_location ]; then
  ...
  echo "Installing nio"
  sudo pip3 install -qqq nio_lite-20171006-py3-none-any.whl
  nio_location="$(which nio_run)"
fi

echo ""
echo "Starting nio"
$nio_location -e envs/mbp.env &
