...
const SelectedRouterCard = ({ client }) => {
  const { name, up_total, down_total, visibleRouterClientList } = client;
  ...
  return (
    <Card id="selectedCard">
      <CardBody className="content-holder">
        ...
        <hr className="mt-1"/>
        {visibleRouterClientList.map((c, i) => (
          <Row key={`${c.name}-${i}`} className="text-muted mb-1">
